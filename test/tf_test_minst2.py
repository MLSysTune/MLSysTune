...
with tf.variable_scope('train'):
    ...
    grad_op = tf.train.SyncReplicasOptimizer(grad_op,
        replicas_to_aggregate=10,
        total_num_replicas=10
    )
    train_op = grad_op.minimize(cross_entropy, global_step=global_step)
    grad_op.get_init_tokens_op()
    queuerunner = tf.get_collection(tf.GraphKeys.QUEUE_RUNNERS)
    b = grad_op._accumulator_list
    assert  True

writer = tf.summary.FileWriter("/tmp/tf_sync_graph")
writer.add_graph(tf.get_default_graph())
writer.close()
ops = tf.get_default_graph().get_operations()
def get_sync_take_gradient_num_required_ops():
    ret = []
    for op in tf.get_default_graph().get_operations():
        if "AccumulatorTakeGradient" in op.name:
            ret.append(op)
    return ret

a = get_sync_take_gradient_num_required_ops()

for op in a :
    op._set_attr("value", tf.AttrValue(tensor=tf.make_tensor_proto(values=5)))
    array_op = tf.fill([10], 1)
    array_op._shape_val = tf.TensorShape([5])
    assert True
# accuracy
