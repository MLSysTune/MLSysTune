...
from test.bo_test.util import get_analysis_logger
...
class GPRGD(GPR):
    ...
    def fit(self, X_train, y_train, X_min, X_max, ridge):  # pylint: disable=arguments-differ
        ...
        with tf.Session(graph=self.graph,
                        config=tf.ConfigProto(
                            intra_op_parallelism_threads=self.num_threads_)) as sess:
            xt_ = tf.get_variable(shape = (X_train[0].shape[0]-1), dtype=tf.float32, name="xt_")
            ...
            sess.run(init)
            xt_augment = tf.concat([xt_, [current_loss_]], 0) #add loss dimension
            ...
            loss = -tf.squeeze(tf.multiply(sig_val, tf.add(tf.multiply(z, dist.cdf(z)), dist.prob(z))))
    ...
    def predict(self, X_test, current_loss, constraint_helper=None,  # pylint: disable=arguments-differ
        ...
        with tf.Session(graph=self.graph,
            ...
            while arr_offset < test_size:
                ...
                for i in range(batch_len):
                    ...
                    sess.run(current_loss_assign_op, feed_dict={current_loss_ph: current_loss})
                    step = 0
