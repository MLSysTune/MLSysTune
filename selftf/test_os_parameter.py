...
def random_generating_config(model):
    ...
    list_ps_tuner_config = lhs_runner.get_batch_lhs_config(30, mock_job_obj)
...
def is_no_next_config(monitor_out_path):
    monitor_output = subprocess.check_output(["tail", "-n","20" ,monitor_out_path])
    regex_end = re.compile(".*No next configuraiton\n")
    m_end = regex_end.search(monitor_output.decode('utf-8'))
    return m_end is not None
...
def wait_finish(job_id):
    ...
    while not os.path.exists(os.path.join(dir,"monitor.out")):
        ...
        if (time.time() - start_time) > best_time or is_diverge(os.path.join(home_path,"monitor.out")) or is_no_next_config(os.path.join(home_path,"monitor.out")):
...
def execute(config):
    ...
    if model == "SVM_BIG":
        ...
        cmd = "%s --target_loss 0.01 " \
              "--script %s %s/selftf/tf_job/nlp/zmwu/bd_lstm/train.py" % (
                  base_cmd,
                  python_exe,
                  home_path)
    elif model == "NLP_transformer_XL":
        cmd = "%s --target_loss 0.01 " \
              "--script %s %s/selftf/tf_job/nlp/zmwu/transformer_xl/train.py" % (
                  base_cmd,
                  python_exe,
                  home_path)
    elif "pytorch" in model:
        cmd = "%s --target_loss 0.01 " \
              "--script %s %s/selftf/tf_job/other_framework/pytorch/launch.py --nnodes=%s --master_addr=%s --model=%s" % (
                  base_cmd,
                  python_exe,
                  home_path,
                  os.getenv("SELFTF_NUM_COMPUTE_NODE"),
                  os.getenv("SELFTF_MASTER_NODE"),
                  model)
    elif "mxnet" in model:
        cmd = "%s --target_loss 0.01 " \
              "--script %s %s/selftf/tf_job/other_framework/mxnet/launch.py -np %s --host-discovery-script \"\'\"cat %s\"\'\" --slots 1 %s %s/selftf/tf_job/other_framework/mxnet/train.py --model=%s" % (
                  base_cmd,
                  python_exe,
                  home_path,
                  os.getenv("SELFTF_NUM_COMPUTE_NODE"),
                  os.path.join(os.getenv("SELFTF_HOME"),"slaves"),
                  python_exe,
                  home_path,
                  model)
    else:
