...
def cnn(x, num_partition):
    ...
    with tf.name_scope('alexnet_cnn') as scope:
        with tf.name_scope('alexnet_cnn_conv1') as inner_scope:
            wcnn1 = tu.weight([11, 11, 3, 96], name='wcnn1', num_partition=num_partition)
            bcnn1 = tu.bias(0.0, [96], name='bcnn1')
            conv1 = tf.add(tu.conv2d(x, wcnn1, stride=(4, 4), padding='SAME'),
                           bcnn1)
            # conv1 = tu.batch_norm(conv1)
            conv1 = tu.relu(conv1)
            norm1 = tu.lrn(conv1, depth_radius=2, bias=1.0, alpha=2e-05,
                           beta=0.75)
            pool1 = tu.max_pool2d(norm1, kernel=[1, 3, 3, 1],
                                  stride=[1, 2, 2, 1], padding='VALID')

        with tf.name_scope('alexnet_cnn_conv2') as inner_scope:
            wcnn2 = tu.weight([5, 5, 96, 256], name='wcnn2', num_partition=num_partition)
            bcnn2 = tu.bias(1.0, [256], name='bcnn2')
            conv2 = tf.add(
                tu.conv2d(pool1, wcnn2, stride=(1, 1), padding='SAME'), bcnn2)
            # conv2 = tu.batch_norm(conv2)
            conv2 = tu.relu(conv2)
            norm2 = tu.lrn(conv2, depth_radius=2, bias=1.0, alpha=2e-05,
                           beta=0.75)
            pool2 = tu.max_pool2d(norm2, kernel=[1, 3, 3, 1],
                                  stride=[1, 2, 2, 1], padding='VALID')

        with tf.name_scope('alexnet_cnn_conv3') as inner_scope:
            wcnn3 = tu.weight([3, 3, 256, 384], name='wcnn3', num_partition=num_partition)
        ...
        with tf.name_scope('alexnet_cnn_conv4') as inner_scope:
            wcnn4 = tu.weight([3, 3, 384, 384], name='wcnn4', num_partition=num_partition)
        ...
        with tf.name_scope('alexnet_cnn_conv5') as inner_scope:
            wcnn5 = tu.weight([3, 3, 384, 256], name='wcnn5', num_partition=num_partition)
            bcnn5 = tu.bias(1.0, [256], name='bcnn5')
            conv5 = tf.add(
                tu.conv2d(conv4, wcnn5, stride=(1, 1), padding='SAME'), bcnn5)
            # conv5 = tu.batch_norm(conv5)
            conv5 = tu.relu(conv5)
            pool5 = tu.max_pool2d(conv5, kernel=[1, 3, 3, 1],
                                  stride=[1, 2, 2, 1], padding='VALID')

        return pool5


def classifier(x, dropout, num_class, num_partition):
    """
	...
	"""
    pool5 = cnn(x, num_partition=num_partition)
    ...
    with tf.name_scope('alexnet_classifier') as scope:
        with tf.name_scope('alexnet_classifier_fc1') as inner_scope:
            wfc1 = tu.weight([flat_dim, 4096], name='wfc1', num_partition=num_partition)
        ...
        with tf.name_scope('alexnet_classifier_fc2') as inner_scope:
            wfc2 = tu.weight([4096, 4096], name='wfc2', num_partition=num_partition)
        ...
        with tf.name_scope('alexnet_classifier_output') as inner_scope:
            wfc3 = tu.weight([4096, num_class], name='wfc3', num_partition=num_partition)
            bfc3 = tu.bias(0.0, [num_class], name='bfc3')
