...
def weight(shape, name, num_partition=1):
    initial = tf.truncated_normal(shape, stddev=0.01)
    w = tf.get_variable(initializer=initial, name=name,
                        partitioner=tf.fixed_size_partitioner(num_partition))
    tf.add_to_collection('weights', w)
    return w


def bias(value, shape, name, num_partition=1):
...
def read_batch(batch_size, images_source, wnid_labels):
    ...
    for i in range(batch_size):
        ...
        class_index = random.randint(0, _class - 1)
...
def preprocess_image(image_path):
    ...
    img = Image.open(tf.gfile.FastGFile(image_path)).convert('RGB')
...
def read_validation_batch(batch_size, validation_source, annotations):
    ...
    with tf.gfile.GFile(annotations) as f:
        gt_idxs = f.readlines()
...
def read_validation_batch_V2(n_val, validation_source, annotations):
    ...
    # reading groundthruths labels
    f = tf.gfile.GFile(annotations)
    for item in f.readlines():
        tf.logging.debug("Chris_debug:" + str(item))
        pic = item.split(",")[1].strip()
        while len(pic) < 5:
            pic = "0" + pic
        idx = item.split(",")[2]
        batch_images_val.append(
            preprocess_image("/root/data/ILSVRC/Data/CLS-LOC/val/ILSVRC2012_val_000" + pic + ".JPEG"))
...
def load_imagenet_meta(meta_path):
    ...
    """
    meta_file = tf.gfile.GFile(meta_path)
...
def read_test_labels(annotations_path):
    ...
    with tf.gfile.GFile(annotations_path) as f:
        gt_idxs = f.readlines()
        gt_idxs = [(int(x.strip()) - 1) for x in gt_idxs]
...
def get_winds(num_class, dir):
    return tf.gfile.ListDirectory(dir)[:num_class]
