...
def lrn(x, depth_radius, bias, alpha, beta):
    return tf.nn.local_response_normalization(x, depth_radius, bias, alpha,
                                              beta)
...
def batch_norm(x):
    ...
    return tf.nn.batch_normalization(x, batch_mean, batch_var, None, None,
                                     epsilon)
...
def read_image(images_folder):
    ...
    image_path = os.path.join(images_folder,
                              random.choice(os.listdir(images_folder)))
...
def read_validation_batch(batch_size, validation_source, annotations):
    ...
    for i in range(batch_size):
        ...
        batch_images_val.append(
            preprocess_image(os.path.join(validation_source, image)))
...
def format_time(time):
    ...
    h, m = divmod(m, 60)
    d, h = divmod(h, 24)
    return ('{:02d}d {:02d}h {:02d}m {:02d}s').format(int(d), int(h), int(m),
                                                      int(s))
