...
class GPR(object):
    ...
    def __init__(self, length_scale=2.0, magnitude=1.0, ridge=1.0, max_train_size=7000,
                 batch_size=3000, num_threads=16, check_numerics=True, debug=False,
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            if self.check_numerics:
                dist_op = tf.debugging.check_numerics(dist_op, "dist_op: ")
            ...
            X_dists = tf.compat.v1.placeholder(tf.float32, name='X_dists')
            ridge_ph = tf.compat.v1.placeholder(tf.float32, name='ridge')
            K_op = mag_var * tf.exp(-X_dists / ls_var)  # pylint: disable=invalid-name
            if self.check_numerics:
                K_op = tf.debugging.check_numerics(K_op, "K_op: ")
            K_ridge_op = K_op + tf.linalg.diag(ridge_ph)
            if self.check_numerics:
                K_ridge_op = tf.debugging.check_numerics(K_ridge_op, "K_ridge_op: ")
            ...
            K = tf.compat.v1.placeholder(tf.float32, name='K')
            K_inv = tf.compat.v1.placeholder(tf.float32, name='K_inv')
            xy_ = tf.compat.v1.placeholder(tf.float32, name='xy_')
            yt_ = tf.compat.v1.placeholder(tf.float32, name='yt_')
            K_inv_op = tf.linalg.inv(K)
            if self.check_numerics:
                K_inv_op = tf.debugging.check_numerics(K_inv_op, "K_inv: ")
            xy_op = tf.matmul(K_inv, yt_)
            if self.check_numerics:
                xy_op = tf.debugging.check_numerics(xy_op, "xy_: ")
            ...
            K2 = tf.compat.v1.placeholder(tf.float32, name="K2")
            K3 = tf.compat.v1.placeholder(tf.float32, name="K3")
            ridge_test_ph = tf.compat.v1.placeholder(tf.float32, name="ridge_test_ph")
            yhat_ = tf.cast(tf.matmul(tf.transpose(K2), xy_), tf.float32)
            if self.check_numerics:
                yhat_ = tf.debugging.check_numerics(yhat_, "yhat_: ")
            sv1 = tf.matmul(tf.transpose(K2), tf.matmul(K_inv, K2))
            if self.check_numerics:
                sv1 = tf.debugging.check_numerics(sv1, "sv1: ")
            sig_val = tf.cast((tf.sqrt(tf.linalg.diag_part(K3 + tf.linalg.diag(ridge_test_ph) - sv1))),
                              tf.float32)
            if self.check_numerics:
                sig_val = tf.debugging.check_numerics(sig_val, "sig_val: ")
            ...
            if self.check_numerics:
                y_best_op = tf.debugging.check_numerics(y_best_op, "y_best_op: ")
            self.ops['y_best_op'] = y_best_op

            sigma = tf.compat.v1.placeholder(tf.float32, name='sigma')
            yhat = tf.compat.v1.placeholder(tf.float32, name='yhat')
...
class GPRGD(GPR):
    ...
    def fit(self, X_train, y_train, X_min, X_max):  # pylint: disable=arguments-differ
        ...
        with tf.compat.v1.Session(graph=self.graph,
            ...
            xt_ph = tf.compat.v1.placeholder(tf.float32)
...
def gd_tf(xs, ys, xt, ridge, length_scale=1.0, magnitude=1.0, max_iter=50):
    ...
    with tf.Graph().as_default():
        ...
        K = magnitude * tf.exp(-tmp / length_scale) + tf.linalg.diag(ridge)
        ...
        x = tf.matmul(tf.linalg.inv(K), ys)
        x = sess.run(x)
        yhat_ = tf.cast(tf.matmul(tf.transpose(K2), x), tf.float32)
        sig_val = tf.cast((tf.sqrt(magnitude - tf.matmul(
            tf.transpose(K2), tf.matmul(tf.linalg.inv(K), K2)))), tf.float32)

        LOG.debug('yhat shape: %s', str(sess.run(yhat_).shape))
        LOG.debug('sig_val shape: %s', str(sess.run(sig_val).shape))
        yhat_ = tf.debugging.check_numerics(yhat_, message='yhat: ')
        sig_val = tf.debugging.check_numerics(sig_val, message='sig_val: ')
        loss = tf.squeeze(tf.subtract(yhat_, sig_val))
        loss = tf.debugging.check_numerics(loss, message='loss: ')
