...
class TensorFlowConfigurationManager(object):
    def __init__(self, get_num_of_node_func, get_num_of_thread_func, learning_rate_range, batch_size_range):
        ...
        MXNET_CPU_WORKER_NTHREADS = TensorFlowConfigMetaData(
            _MXNET_CPU_WORKER_NTHREADS,
            min_func=lambda: 1, max_func=lambda: 5,
            is_integer=True, default=1,
            is_binary=True)
        self.add_config_meta_data(MXNET_CPU_WORKER_NTHREADS)

        MXNET_CPU_NNPACK_NTHREADS = TensorFlowConfigMetaData(
            _MXNET_CPU_NNPACK_NTHREADS,
            min_func=lambda: 1, max_func=lambda: 6,
            is_integer=True, default=4,
            is_binary=True)
        self.add_config_meta_data(MXNET_CPU_NNPACK_NTHREADS)

        MXNET_MP_WORKER_NTHREADS = TensorFlowConfigMetaData(
            _MXNET_MP_WORKER_NTHREADS,
            min_func=lambda: 1, max_func=lambda: 5,
            is_integer=True, default=1,
            is_binary=True)
        self.add_config_meta_data(MXNET_MP_WORKER_NTHREADS)

        MXNET_EXEC_ENABLE_INPLACE = TensorFlowConfigMetaData(
            _MXNET_EXEC_ENABLE_INPLACE,
            min_func=lambda: 0, max_func=lambda: 1,
            is_integer=True, default=1,
            is_binary=True)
        self.add_config_meta_data(MXNET_EXEC_ENABLE_INPLACE)

        MXNET_EXEC_NUM_TEMP = TensorFlowConfigMetaData(
            _MXNET_EXEC_NUM_TEMP,
            min_func=lambda: 1, max_func=lambda: 3,
            is_integer=True, default=1,
            is_binary=True)
        self.add_config_meta_data(MXNET_EXEC_NUM_TEMP)

        MXNET_GPU_MEM_POOL_TYPE = TensorFlowConfigMetaData(
            _MXNET_GPU_MEM_POOL_TYPE,
            min_func=lambda: 0, max_func=lambda: 2,
            is_integer=True, default=0,
            is_binary=True)
        self.add_config_meta_data(MXNET_GPU_MEM_POOL_TYPE)

        MXNET_ENGINE_TYPE = TensorFlowConfigMetaData(
            _MXNET_ENGINE_TYPE,
            min_func=lambda: 0, max_func=lambda: 2,
            is_integer=True, default=1,
            is_binary=True)
        self.add_config_meta_data(MXNET_ENGINE_TYPE)

        MXNET_KVSTORE_USETREE = TensorFlowConfigMetaData(
            _MXNET_KVSTORE_USETREE,
            min_func=lambda: 0, max_func=lambda: 1,
            is_integer=True, default=0,
            is_binary=True)
        self.add_config_meta_data(MXNET_KVSTORE_USETREE)

        MXNET_UPDATE_ON_KVSTORE = TensorFlowConfigMetaData(
            _MXNET_UPDATE_ON_KVSTORE,
            min_func=lambda: 0, max_func=lambda: 1,
            is_integer=True, default=1,
            is_binary=True)
        self.add_config_meta_data(MXNET_UPDATE_ON_KVSTORE)

        MXNET_CPU_TEMP_COPY = TensorFlowConfigMetaData(
            _MXNET_CPU_TEMP_COPY,
            min_func=lambda: 1, max_func=lambda: 5,
            is_integer=True, default=4,
            is_binary=True)
        self.add_config_meta_data(MXNET_CPU_TEMP_COPY)

        MXNET_GPU_PARALLEL_RAND_COPY = TensorFlowConfigMetaData(
            _MXNET_GPU_PARALLEL_RAND_COPY,
            min_func=lambda: 1, max_func=lambda: 5,
            is_integer=True, default=4,
            is_binary=True)
        self.add_config_meta_data(MXNET_GPU_PARALLEL_RAND_COPY)
    ...
    def get_tf_config_by_key_config(self, ps_num,
        ...
        return PSTunerConfiguration(
            ...
            MXNET_CPU_NNPACK_NTHREADS=MXNET_CPU_NNPACK_NTHREADS,
            MXNET_MP_WORKER_NTHREADS=MXNET_MP_WORKER_NTHREADS,
            MXNET_EXEC_ENABLE_INPLACE=MXNET_EXEC_ENABLE_INPLACE,
            MXNET_EXEC_NUM_TEMP=MXNET_EXEC_NUM_TEMP,
            MXNET_GPU_MEM_POOL_TYPE=MXNET_GPU_MEM_POOL_TYPE,
            MXNET_ENGINE_TYPE=MXNET_ENGINE_TYPE,
            MXNET_KVSTORE_USETREE=MXNET_KVSTORE_USETREE,
            MXNET_UPDATE_ON_KVSTORE=MXNET_UPDATE_ON_KVSTORE,
            MXNET_CPU_TEMP_COPY=MXNET_CPU_TEMP_COPY,
            MXNET_GPU_PARALLEL_RAND_COPY=MXNET_GPU_PARALLEL_RAND_COPY
        )
...
class TFConfigUtil(object):
    def __init__(self, tf_config_manager, customize_config_sequence=None):
        """
        ...
        if customize_config_sequence is not None:
            self.config_sequence = customize_config_sequence
    ...
    def get_range_matrix(self, tf_config_meta_data_map):
        ...
        for x in self.config_sequence:
            ...
            ret_array.append([
                ...
                config_meta_data.max_func()() if callable(config_meta_data.max_func()) else config_meta_data.max_func()
            ])
        return ret_array
    ...
    def config_vector_to_config_obj(self, config_vector, job_obj):
        ...
        MKL_DYNAMIC=config_vector[self.config_sequence.index(
            _MKL_DYNAMIC)]
        MXNET_CPU_WORKER_NTHREADS=config_vector[self.config_sequence.index(
            _MXNET_CPU_WORKER_NTHREADS)]
        MXNET_CPU_NNPACK_NTHREADS=config_vector[self.config_sequence.index(
            _MXNET_CPU_NNPACK_NTHREADS)]
        MXNET_MP_WORKER_NTHREADS=config_vector[self.config_sequence.index(
            _MXNET_MP_WORKER_NTHREADS)]
        MXNET_EXEC_ENABLE_INPLACE=config_vector[self.config_sequence.index(
            _MXNET_EXEC_ENABLE_INPLACE)]
        MXNET_EXEC_NUM_TEMP=config_vector[self.config_sequence.index(
            _MXNET_EXEC_NUM_TEMP)]
        MXNET_GPU_MEM_POOL_TYPE=config_vector[self.config_sequence.index(
            _MXNET_GPU_MEM_POOL_TYPE)]
        MXNET_ENGINE_TYPE=config_vector[self.config_sequence.index(
            _MXNET_ENGINE_TYPE)]
        MXNET_KVSTORE_USETREE=config_vector[self.config_sequence.index(
            _MXNET_KVSTORE_USETREE)]
        MXNET_UPDATE_ON_KVSTORE=config_vector[self.config_sequence.index(
            _MXNET_UPDATE_ON_KVSTORE)]
        MXNET_CPU_TEMP_COPY=config_vector[self.config_sequence.index(
            _MXNET_CPU_TEMP_COPY)]
        MXNET_GPU_PARALLEL_RAND_COPY=config_vector[self.config_sequence.index(
            _MXNET_GPU_PARALLEL_RAND_COPY)]
    ...
    def tf_config_to_vector(self, tf_config):
        ...
        ret = []
        for x in self.config_sequence:
            if not hasattr(tf_config, x):
                ret.append(self.tf_config_meta_data_map[x].default)
            else:
    ...
    def denormalize_config_vector(self, x):
        ...
        for idx, config_name in enumerate(self.config_sequence):
            ...
            if metadata.is_categorical or metadata.is_integer:
                ...
                max_value = metadata.max_func()() if callable(metadata.max_func()) else metadata.max_func()
    ...
    def _get_min_max_vectors(tf_config_meta_data_map, config_sequence, categorical_encoder):
        ...
        for idx, config_name in enumerate(config_sequence):
            ...
            max.append(metadata.max_func()() if callable(metadata.max_func()) else metadata.max_func())
