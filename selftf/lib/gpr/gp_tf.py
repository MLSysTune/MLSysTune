...
from selftf.lib.gpr.util import get_analysis_logger
    ...
    def fit(self, X_train, y_train, ridge=1.0):
        ...
        with tf.Session(graph=self.graph,
            ...
            for i in range(sample_size):
                X_dists[i] = sess.run(dist_op, feed_dict={v1: self.X_train[i],
                                                          v2: self.X_train})
            ...
            self.K = sess.run(K_ridge_op,
                              feed_dict={X_dists_ph: X_dists, ridge_ph: ridge})
...
class GPRGD(GPR):
    ...
    def fit(self, X_train, y_train, X_min, X_max,
        ridge):  # pylint: disable=arguments-differ
        ...
        with tf.Session(graph=self.graph,
                        config=tf.ConfigProto(
                            intra_op_parallelism_threads=self.num_threads_)) as sess:
            xt_ = tf.get_variable(shape=(X_train[0].shape[0] - 1),
                                  dtype=tf.float32, name="xt_")
            xt_ph = tf.placeholder(tf.float32)
            xt_assign_op = xt_.assign(xt_ph)
            current_loss_ = tf.Variable(0, dtype=tf.float32, trainable=False)
            current_loss_ph = tf.placeholder(tf.float32)
            current_loss_assign_op = current_loss_.assign(current_loss_ph)
            init = tf.global_variables_initializer()
            sess.run(init)
            xt_augment = tf.concat([xt_, [current_loss_]],
                                   0)  # add loss dimension
            K2_mat = tf.transpose(tf.expand_dims(tf.sqrt(tf.reduce_sum(tf.pow(
                tf.subtract(xt_augment, self.X_train), 2), 1)), 0))
            if self.check_numerics is True:
                K2_mat = tf.check_numerics(K2_mat, "K2_mat: ")
            K2__ = tf.cast(self.magnitude * tf.exp(-K2_mat / self.length_scale),
                           tf.float32)
            if self.check_numerics is True:
            ...
            # TODO: when sigma=0, ignore z, loss should be 0
            y_ref = tf.constant(y_train[-1, 0], dtype=tf.float32)
            z = tf.divide(tf.subtract(y_ref, yhat_gd), sig_val)
            dist = tf.distributions.Normal(loc=0., scale=1.)
            # ei = sigma*(z*cdf(z) + pdf(z))
            # maximize ei is equivalent to minimize -ei
            loss = -tf.squeeze(tf.multiply(sig_val,
                                           tf.add(tf.multiply(z, dist.cdf(z)),
                                                  dist.prob(z))))
            ...
            self.vars['current_loss_'] = current_loss_
            self.vars['current_loss_ph'] = current_loss_ph
            self.ops['current_loss_assign_op'] = current_loss_assign_op
    ...
    def predict(self, X_test, current_loss, constraint_helper=None,
        ...
        with tf.Session(graph=self.graph,
            ...
            while arr_offset < test_size:
                ...
                xt_ = self.vars['xt_']
                current_loss_ = self.vars['current_loss_']
                init = tf.global_variables_initializer()
                sess.run(init)
                ...
                current_loss_ph = self.vars['current_loss_ph']
                current_loss_assign_op = self.ops['current_loss_assign_op']
                ...
                for i in range(batch_len):
                    ...
                    sess.run(current_loss_assign_op,
                             feed_dict={current_loss_ph: current_loss})
                    step = 0
...
def gd_tf(xs, ys, xt, ridge, length_scale=1.0, magnitude=1.0, max_iter=50):
    ...
    with tf.Graph().as_default():
        ...
        sig_val = tf.cast((tf.sqrt(magnitude - tf.matmul(
            tf.transpose(K2), tf.matmul(tf.matrix_inverse(K), K2)))),
                          tf.float32)
