...
class PSTunerConfiguration(object):
    def __init__(self, num_ps=1,
                 ...
                 MXNET_EXEC_ENABLE_INPLACE=1,
                 MXNET_EXEC_NUM_TEMP=1,
                 MXNET_GPU_MEM_POOL_TYPE=0,
                 MXNET_ENGINE_TYPE=1,
                 MXNET_KVSTORE_USETREE=0,
                 MXNET_UPDATE_ON_KVSTORE=1,
                 MXNET_CPU_TEMP_COPY=4,
                 MXNET_GPU_PARALLEL_RAND_COPY=4,
                 py_dict=None
                 ):
        self.ps_num = num_ps
        ...
        self.MXNET_CPU_WORKER_NTHREADS = MXNET_CPU_WORKER_NTHREADS
        self.MXNET_CPU_NNPACK_NTHREADS = MXNET_CPU_NNPACK_NTHREADS
        self.MXNET_MP_WORKER_NTHREADS = MXNET_MP_WORKER_NTHREADS
        self.MXNET_EXEC_ENABLE_INPLACE = MXNET_EXEC_ENABLE_INPLACE
        self.MXNET_EXEC_NUM_TEMP = MXNET_EXEC_NUM_TEMP
        self.MXNET_GPU_MEM_POOL_TYPE = MXNET_GPU_MEM_POOL_TYPE
        self.MXNET_ENGINE_TYPE = MXNET_ENGINE_TYPE
        self.MXNET_KVSTORE_USETREE = MXNET_KVSTORE_USETREE
        self.MXNET_UPDATE_ON_KVSTORE = MXNET_UPDATE_ON_KVSTORE
        self.MXNET_CPU_TEMP_COPY = MXNET_CPU_TEMP_COPY
        self.MXNET_GPU_PARALLEL_RAND_COPY = MXNET_GPU_PARALLEL_RAND_COPY
...
def get_default_learning_rate_batch_size_optimizer(model):
    ...
    if model == "CNN":
    ...
    elif model == 'NLP_transformer_XL':
        defalt_learning_rate = 0.001
        default_batch_size = 32
        default_optimizer = "Adam"
    elif model == 'NLP_bert':
        defalt_learning_rate = 0.001
        ...
        default_batch_size = 128
        default_optimizer = "Adam"
    elif 'pytorch' in model:
        defalt_learning_rate = 0.01
...
def get_default_learning_rate_batch_size_optimizer_by_framework(framework):
...
def random_generating_config(framework, num_samples=1):
    ...
    tf_cfg_manager = TensorFlowConfigurationManager(
        lambda :int(os.getenv("SELFTF_NUM_COMPUTE_NODE")),
        lambda :int(os.getenv("SELFTF_NUM_THREAD")),
        [0,0],
        [0,0]
    )
    ...
    mock_job_obj = Job()
    list_ps_tuner_config = lhs_runner.get_batch_lhs_config(num_samples, mock_job_obj)

    defalt_learning_rate, default_batch_size, default_optimizer = \
        get_default_learning_rate_batch_size_optimizer_by_framework(framework)

    for config in list_ps_tuner_config:
        config.learning_rate = defalt_learning_rate
        config.batch_size = default_batch_size
        config.optimizer = get_optimizer_by_name(default_optimizer)

    if num_samples == 1:
        return list_ps_tuner_config[0].__dict__
    else:
        return list_ps_tuner_config

def get_framework(path):
    frameworks = ["tensorflow", "torch", "mxnet"]
...
_MXNET_CPU_WORKER_NTHREADS = "MXNET_CPU_WORKER_NTHREADS"
_MXNET_CPU_NNPACK_NTHREADS = "MXNET_CPU_NNPACK_NTHREADS"
_MXNET_MP_WORKER_NTHREADS = "MXNET_MP_WORKER_NTHREADS"
_MXNET_EXEC_ENABLE_INPLACE = "MXNET_EXEC_ENABLE_INPLACE"
_MXNET_EXEC_NUM_TEMP = "MXNET_EXEC_NUM_TEMP"
_MXNET_GPU_MEM_POOL_TYPE = "MXNET_GPU_MEM_POOL_TYPE"
_MXNET_ENGINE_TYPE = "MXNET_ENGINE_TYPE"
_MXNET_KVSTORE_USETREE = "MXNET_KVSTORE_USETREE"
_MXNET_UPDATE_ON_KVSTORE = "MXNET_UPDATE_ON_KVSTORE"
_MXNET_CPU_TEMP_COPY = "MXNET_CPU_TEMP_COPY"
_MXNET_GPU_PARALLEL_RAND_COPY = "MXNET_GPU_PARALLEL_RAND_COPY"